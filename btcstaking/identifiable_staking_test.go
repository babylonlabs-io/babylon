...
// Property: Every staking tx generated by our generator should be properly parsed by
// our parser
func FuzzGenerateAndParseValidV0MultisigStakingTransaction(f *testing.F) {
	// lot of seeds as test is pretty fast and we want to test a lot of different values
	datagen.AddRandomSeedsToFuzzer(f, 1000)
	f.Fuzz(func(t *testing.T, seed int64) {
		r := rand.New(rand.NewSource(seed))
		// 1 - 5 stakers
		numStakerKeys := uint32(r.Int31n(5) + 1)
		stakerQuorum := uint32(r.Intn(int(numStakerKeys)) + 1)
		// 3 - 10 covenants
		numCovenantKeys := uint32(r.Int31n(7) + 3)
		quorum := uint32(r.Intn(int(numCovenantKeys)) + 1)
		stakingAmount := btcutil.Amount(r.Int63n(1000000000) + 10000)
		...
		sc := GenerateMultisigTestScenario(r, t, numStakerKeys, stakerQuorum, 1, numCovenantKeys, quorum, stakingAmount, stakingTime)

		outputs, err := btcstaking.BuildV0IdentifiableMultisigStakingOutputs(
			tag,
			sc.StakerPublicKeys(),
			stakerQuorum,
			sc.FinalityProviderKeys[0].PubKey(),
		...
		parsedTx, err := btcstaking.ParseV0MultisigStakingTx(
			tx,
			tag,
			sc.StakerPublicKeys(),
			stakerQuorum,
		...
		require.Equal(t, schnorr.SerializePubKey(sc.StakerPublicKeys()[0]), parsedTx.OpReturnData.StakerPublicKey.Marshall())
		require.Equal(t, schnorr.SerializePubKey(sc.FinalityProviderKeys[0].PubKey()), parsedTx.OpReturnData.FinalityProviderPublicKey.Marshall())
	})
}
