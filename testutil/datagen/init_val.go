...
func InitializeNodeValidatorFiles(config *cfg.Config, addr sdk.AccAddress) (string, *bb.ValidatorKeys, error) {
	return InitializeNodeValidatorFilesFromMnemonic(config, "", addr)
}

func InitializeNodeValidatorFilesFromMnemonic(config *cfg.Config, mnemonic string, addr sdk.AccAddress) (nodeID string, valKeys *bb.ValidatorKeys, err error) {
	...
	blsKeyFile := bb.DefaultBlsKeyFile(config.RootDir)
	blsPasswordFile := bb.DefaultBlsPasswordFile(config.RootDir)
	if err := bb.EnsureDirs(cmtKeyFile, cmtStateFile, blsKeyFile, blsPasswordFile); err != nil {
	...
	valKeys, err = bb.NewValidatorKeys(filePV.Key.PrivKey, bls.Key.PrivKey)
	if err != nil {
		return "", nil, err
	}
