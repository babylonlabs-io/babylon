...
- `ProofBsnHeaderInEpoch`: Proof that the header of the PoS blockchain is
...
message ChainInfo {
  ...
  // latest_header is the latest header in the BSN's canonical chain
...
message IndexedHeader {
  ...
  // (hash, height) jointly provides the position of the header on the BSN's ledger
  uint64 height = 3;
  // time is the timestamp of this header on the BSN's ledger.
  // It is needed for the BSN to unbond all mature validators/delegations
...
message ProofFinalizedChainInfo {
  /*
    ...
    BTC-finalized
  */
  // proof_bsn_header_in_epoch is the proof that the BSN header is timestamped
  // within a certain epoch
  tendermint.crypto.ProofOps proof_bsn_header_in_epoch = 1;
