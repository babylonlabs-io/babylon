...
The Zone Concierge module binds to the `zoneconcierge` port for IBC communication.

### LastSentBTCSegment

The [last sent segment storage](./keeper/epochs.go) maintains the last segment
of BTC headers that was broadcasted to other light clients. This is used for
determining which headers to send next and handling reorgs.

### SealedEpochProof

The [sealed epoch proof storage](./keeper/epochs.go) stores proofs that each
epoch is sealed by validator signatures. These proofs are generated when an
epoch's raw checkpoint is sealed and are used in BTC timestamp construction.
...
[x/zoneconcierge/keeper/ibc_header_decorator.go](./keeper/ibc_header_decorator.go), and
works as follows.
