...
func (k Keeper) HandleFPStateUpdates(ctx context.Context, prevDc, newDc *ftypes.VotingPowerDistCache, state *ftypes.ProcessingState) {
	...
	for _, fp := range newlyActiveFPs {
		...
		k.processHooksFp(sdkCtx, state.FpByBtcPk, *fp.BtcPk, prevStatus, newState)
	}
	...
	for _, fp := range newlyInactiveFPs {
		k.processInactiveFp(ctx, state, fpStates, fp)
	}
}
...
func (k Keeper) processInactiveFp(
	...
	fpStates map[string]ftypes.FinalityProviderState,
	...
	k.processHooksFp(sdkCtx, state.FpByBtcPk, *fp.BtcPk, fp.Status, newStatus)
...
func (k Keeper) ProcessAllPowerDistUpdateEvents(
	...
	prevFpByBtcPk := dc.FpsByBtcPkHex()
	for btcHeight := lastBTCTip; btcHeight <= curBTCTip; btcHeight++ {
		k.processEventsAtHeight(sdkCtx, btcHeight, state, prevFpByBtcPk)
	}
...
func (k Keeper) processEventsAtHeight(
	...
	btcHeight uint32,
	state *ftypes.ProcessingState,
	prevFpByBtcPkHex map[string]*ftypes.FinalityProviderDistInfo,
) {
	...
	for ; iter.Valid(); iter.Next() {
		...
		case *types.EventPowerDistUpdate_SlashedFp:
			...
			k.processHooksFp(ctx, state.FpByBtcPk, *typedEvent.SlashedFp.Pk, fpDstInfoPrevStatus(prevFpByBtcPkHex, typedEvent.SlashedFp.Pk), btcstktypes.FinalityProviderStatus_FINALITY_PROVIDER_STATUS_SLASHED)
		case *types.EventPowerDistUpdate_JailedFp:
			// record jailed fps
			types.EmitJailedFPEvent(ctx, typedEvent.JailedFp.Pk)
			k.processHooksFp(ctx, state.FpByBtcPk, *typedEvent.JailedFp.Pk, fpDstInfoPrevStatus(prevFpByBtcPkHex, typedEvent.JailedFp.Pk), btcstktypes.FinalityProviderStatus_FINALITY_PROVIDER_STATUS_JAILED)
...
func fpDstInfoPrevStatus(prevFpByBtcPkHex map[string]*ftypes.FinalityProviderDistInfo, fpBtcPk *bbn.BIP340PubKey) btcstktypes.FinalityProviderStatus {
	fp, found := prevFpByBtcPkHex[fpBtcPk.MarshalHex()]
	if !found {
...
func (k Keeper) processPowerDistUpdateEventUnbond(
	...
	sdkCtx := sdk.UnwrapSDKContext(ctx)
	k.processHooksBtcDel(sdkCtx, state.FpByBtcPk, btcDel, func(fp *types.FinalityProvider, del sdk.AccAddress, fpSecuresBabylon bool, sats uint64) {
		err := k.hooks.AfterBtcDelegationUnbonded(sdkCtx, fp.Address(), del, fpSecuresBabylon, sats)
		if err != nil {
...
func (k Keeper) processPowerDistUpdateEventActive(
	...
	k.processHooksBtcDel(sdkCtx, state.FpByBtcPk, btcDel, func(fp *types.FinalityProvider, del sdk.AccAddress, fpSecuresBabylon bool, sats uint64) {
		err := k.hooks.AfterBtcDelegationActivated(sdkCtx, fp.Address(), del, fpSecuresBabylon, sats)
		if err != nil {
			k.Logger(sdkCtx).Error(
				"failed to add event of activated BTC delegation",
				"blockHeight", sdkCtx.HeaderInfo().Height,
...
// processHooksBtcDel loads Babylon FPs from the given BTC delegation
...
func (k Keeper) processHooksBtcDel(
	ctx sdk.Context,
	fpByBtcPkHex map[string]*types.FinalityProvider,
	btcDel *types.BTCDelegation,
	f func(fp *types.FinalityProvider, del sdk.AccAddress, fpSecuresBabylon bool, sats uint64),
) {
	...
	for _, fpBTCPK := range btcDel.FpBtcPkList {
		...
		if err != nil {
			k.Logger(ctx).Error(
				"failed process the btc del hooks for the given fp",
				err,
				"fp_btc_pk", fpBtcPk,
				"del_addr", btcDel.StakerAddr,
			)
			panic(err)
		}
		f(fp, delAddr, fp.SecuresBabylonGenesis(ctx), btcDel.TotalSat)
	}
}

// processHooksFp loads Babylon FP from the given fp btc pk
// and executes the given function over the loaded FP
// NOTE:
//   - The function makes uses of the fpByBtcPkHex cache
//   - Calls the hook AfterFpStatusChange
func (k Keeper) processHooksFp(
	ctx sdk.Context,
	fpByBtcPkHex map[string]*types.FinalityProvider,
	fpBTCPK bbn.BIP340PubKey,
	prevStatus, newStatus btcstktypes.FinalityProviderStatus,
) {
	fpBtcPk := fpBTCPK.MarshalHex()
	fp, err := k.loadFP(ctx, fpByBtcPkHex, fpBtcPk)
	if err != nil {
		k.Logger(ctx).Error(
			"failed process the hooks for the given fp",
			err,
			"fp_btc_pk", fpBtcPk,
		)
		panic(fmt.Errorf("failed to load fp %s: %w", fpByBtcPkHex, err))
	}

	fpSecuresBabylon := fp.SecuresBabylonGenesis(ctx)
	err = k.hooks.AfterFpStatusChange(ctx, fp.Address(), fpSecuresBabylon, prevStatus, newStatus)
	if err != nil {
		panic(fmt.Errorf("failed to call hook AfterFpStatusChange for fp %s with status %s: %w", fpByBtcPkHex, newStatus.String(), err))
	}
}
