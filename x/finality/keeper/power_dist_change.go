	...
	for _, event := range events {
		...
		case *types.EventPowerDistUpdate_BtcDelStateUpdate:
			...
			if delEvent.NewState == types.BTCDelegationStatus_ACTIVE {
				...
				err := k.BTCStakingKeeper.AddDelStaking(ctx, delAddr, math.NewIntFromUint64(btcDel.TotalSat))
				if err != nil {
					panic(err)
				}
				...
				delAddr := sdk.MustAccAddressFromBech32(btcDel.StakerAddr)
				err := k.BTCStakingKeeper.SubDelStaking(ctx, delAddr, math.NewIntFromUint64(btcDel.TotalSat))
				if err != nil {
					panic(err)
				}
			}
