...
func FuzzSetBTCStakingEventStore_NewFp(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		for _, evFp := range evs.GetNewFp() {
			...
			require.Equal(t, fp.MasterPubRand, evFp.MasterPubRand)
			require.Equal(t, uint64(10), evFp.RegisteredEpoch)
...
func FuzzSetBTCStakingEventStore_ActiveDel(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, seed int64) {
		...
		h.CheckpointingKeeper.EXPECT().GetLastFinalizedEpoch(gomock.Any()).Return(babylonFp.RegisteredEpoch).AnyTimes()
