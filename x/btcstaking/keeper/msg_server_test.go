...
func FuzzMultiStaking_AddCovenantSigs(f *testing.F) {
	datagen.AddRandomSeedsToFuzzer(f, 10)
	f.Fuzz(func(t *testing.T, seed int64) {
		r := rand.New(rand.NewSource(seed))
		ctrl := gomock.NewController(t)
...
func FuzzMultiStaking_BTCUndelegate(f *testing.F) {
	datagen.AddRandomSeedsToFuzzer(f, 10)
	f.Fuzz(func(t *testing.T, seed int64) {
		r := rand.New(rand.NewSource(seed))
		ctrl := gomock.NewController(t)
		...
		// covenant signatures to activate the delegation
		h.CreateCovenantSigs(r, covenantSKs, msgCreateBTCDel, actualDel, 10)
		...
		// unbond the multistaked delegation
		_, err = h.MsgServer.BTCUndelegate(h.Ctx, msg)
		h.NoError(err)
