...
func TestHookStaking_AfterDelegationModified(t *testing.T) {
	...
	// Mock costakerModified call - expect delta of +500
	expectedDelta := math.NewInt(500)
	mockIctvK.EXPECT().AccumulateRewardGaugeForCostaker(gomock.Any(), gomock.Eq(delAddr), gomock.Any()).Times(0) // not called in hook

	// Set default params
	err := k.SetParams(ctx, types.DefaultParams())
	require.NoError(t, err)

	// Initialize rewards system to avoid errors in costakerModified
...
func TestHookStaking_AfterDelegationModified_NoCachedAmount(t *testing.T) {
	...
	// Verify the costaker tracker was updated (delta should be +1000 since before was 0)
	tracker, err := k.GetCostakerRewards(ctx, delAddr)
	require.NoError(t, err)
